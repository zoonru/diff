<?php
/**
 * Shows diff of array of lines.
 * usage: (new Diff)->compare(['aaa', 'bbb'], ['bbb']);
 * returns array of arrays, containing 'copy', 'add' and 'delete' keys
 *
 ...
 *
 * Origin: Text_Diff-1.1.1 pear package Diff/Engine/native.php
 * Refactored by Roman Zaykin <roman@noframework.com>
 *
...
class Diff {
	...
	public function compare($from_lines, $to_lines, $is_copy_include = false) {
		...
		while ($xi < $n_from || $yi < $n_to) {
			...
			if ($delete && $add) {
				$edits[] = ['delete' => $delete, 'add' => $add];
				$total += \count($delete) + \count($add);
			} elseif ($delete) {
				$edits[] = ['delete' => $delete];
				$total += \count($delete);
			} elseif ($add) {
		...
		return ['edits' => $edits, 'total' => $total];
	}
	 ...
	 */
	protected function compareseq($xoff, $xlim, $yoff, $ylim): void {
